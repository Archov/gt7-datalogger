...
| GET | `/api/laps/{id}/export.csv` | MoTeC-compatible CSV (one row per tick; every recorded channel is included with units, while unavailable optional channels are omitted) |
