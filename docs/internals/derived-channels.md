...
| `velocity_x`, `velocity_y`, `velocity_z` | native world velocity (Y is vertical) | m/s |
...
| --- | --- | --- | --- |
| `steering_wheel_rad` | packet B | `wheel_rotation` as broadcast | rad |
| `sway` | packet B | lateral acceleration, raw | see below |
| `surge` | packet B | longitudinal acceleration, raw | see below |
| `heave` | packet B | vertical acceleration, raw | see below |
| `throttle_filtered` | packet ~ | filtered throttle ÷ 2.55 | % |
| `brake_filtered` | packet ~ | filtered brake ÷ 2.55 | % |
...
| **TCS cut** | `max(0, throttle − throttle_filtered)` % — how much throttle traction control took away |
| **ABS release** | `max(0, brake − brake_filtered)` % — how much brake pressure ABS gave back |
