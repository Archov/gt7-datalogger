...
# remain usable while new recordings expose one descriptive schema.
LEGACY_SAMPLE_ALIASES = {
    "steer": "steering_wheel_rad",
    "acc_lat": "sway",
    "acc_long": "surge",
    "acc_vert": "heave",
    "throttle_f": "throttle_filtered",
    "brake_f": "brake_filtered",
}
...
def normalize_sample_columns(samples: dict[str, list[float]]) -> dict[str, list[float]]:
    """Translate persisted/imported pre-merge sample keys in place."""
    for legacy, canonical in LEGACY_SAMPLE_ALIASES.items():
        legacy_values = samples.pop(legacy, None)
        if canonical not in samples and legacy_values is not None:
            samples[canonical] = legacy_values
    return samples
...
def prune_optional(samples: dict[str, list[float]]) -> list[str]:
    ...
    it, so a lap recorded on packet A leaves extension channels empty and a lap that
