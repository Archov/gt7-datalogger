...
from app.processing.laps import STORED_SAMPLE_COLUMNS, VELOCITY_CHANNELS
...
CSV_CHANNELS = (
    ...
    # Upstream compatibility aliases used by the G-G and intervention views.
...
def _validate_import_samples(samples: dict[str, list[float]]) -> dict[str, list[float]]:
    samples = {k: v for k, v in samples.items() if k in STORED_SAMPLE_COLUMNS}
...
EXTRA_COMPARE_COLUMNS = tuple(
    c for c in STORED_SAMPLE_COLUMNS if c not in COMPARE_COLUMNS and c != "dist"
)
