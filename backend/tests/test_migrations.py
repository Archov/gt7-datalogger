...
async def test_pre_alembic_feature_branch_metadata_columns_are_preserved(tmp_path) -> None:
    """The feature branch added its metadata columns before Alembic existed.

    Stamping that database at the baseline must not make 0003 add them again.
    """
    path = tmp_path / "feature-branch.db"
    con = sqlite3.connect(path)
    con.executescript(V1_SCHEMA)
    con.executescript(
        "ALTER TABLE sessions ADD COLUMN raw_archive_meta_json "
        "TEXT NOT NULL DEFAULT '';"
        "ALTER TABLE laps ADD COLUMN telemetry_meta_json TEXT NOT NULL DEFAULT '';"
        "UPDATE sessions SET raw_archive_meta_json = '{\"status\":\"complete\"}';"
        "UPDATE laps SET telemetry_meta_json = '{\"packet_format\":\"C\"}';"
    )
    con.commit()
    con.close()
    ...
    await init_db(engine)
    repo = Repository(make_session_factory(engine))
    lap = await repo.get_lap(1, with_samples=False)
    archive = await repo.get_session_archive_metadata(1)
    await engine.dispose()

    assert _revision(path) == _head()
    assert archive == {"status": "complete"}
    assert lap is not None and lap["telemetry_meta"] == {"packet_format": "C"}
