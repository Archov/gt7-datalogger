...
async def test_refuel_migration_repairs_net_gain_pit_lap_once(tmp_path) -> None:
    ...
    con = sqlite3.connect(path)
    fuel_consumed, lap_revision = con.execute(
        "SELECT fuel_consumed,metrics_revision FROM laps WHERE id=1"
    ).fetchone()
    session_revision = con.execute(
        "SELECT metrics_revision FROM sessions WHERE id=1"
    ).fetchone()[0]
    con.close()
    assert fuel_consumed == pytest.approx(5.0)
    assert lap_revision == 2
    assert session_revision == 2
