...
def test_prune_drops_short_optional_columns() -> None:
    ...
    for i in range(10):
        ...
        s["steering_wheel_rad"].append(0.1)  # complete
        if i < 4:
            s["sway"].append(0.5)  # started late / stopped early

    dropped = prune_optional(s)
    assert "sway" in dropped and "surge" in dropped
    assert "steering_wheel_rad" not in dropped
    assert "steering_wheel_rad" in s and "sway" not in s
...
async def test_packet_a_records_no_optional_columns(client) -> None:
    ...
    for column in (
        column for group in ("B", "~", "C") for column in OPTIONAL_SAMPLE_GROUPS[group]
    ):
...
async def test_packet_c_records_every_optional_column(client) -> None:
    ...
    assert samples["steering_wheel_rad"][0] == pytest.approx(0.42)
    # Filtered pedals arrive as percentages, like their raw counterparts.
    assert max(samples["throttle_filtered"]) == pytest.approx(140 / 2.55, abs=0.1)
    assert max(samples["brake_filtered"]) == pytest.approx(180 / 2.55, abs=0.1)
...
async def test_packet_b_stops_at_the_accelerometer(client) -> None:
    ...
    should carry exactly that, not a half-filled throttle_filtered."""
    c, service = client
    await drive(service, fmt="B")
    lap = (await c.get("/api/laps")).json()[0]
    samples = (await c.get(f"/api/laps/{lap['id']}")).json()["samples"]
    assert {"steering_wheel_rad", "sway", "surge", "heave"} <= samples.keys()
    assert "throttle_filtered" not in samples and "brake_filtered" not in samples
...
async def test_csv_export_carries_the_new_channels(client) -> None:
    ...
    for name in ("Steering Wheel", "Sway", "Filtered Throttle", "Filtered Brake"):
...
def _synthetic(scale_lat: float, scale_long: float, n: int = 1200) -> dict[str, list[float]]:
    ...
    for i in range(n):
        ...
        s["sway"].append(-(v * omega) * scale_lat)
        s["surge"].append(dv * scale_long)
    return s
...
def test_calibration_declines_when_the_lap_proves_nothing() -> None:
    ...
    s = _synthetic(scale_lat=1.0, scale_long=1.0)
    s["sway"] = [((i * 37) % 11) - 5.0 for i in range(len(s["t"]))]  # noise
